...
export class QuoteDetailsService {
  ...
  async updateUp(id: string, updateQuoteDetailDto: CreateQuoteDetailDto, save: number, user: User,) {
    ...
    let dataMarkingServices: any[] = updateQuoteDetailDto.markingServices;

    console.log(dataMarkingServices)
    ...
    if (quoteDetailRefProduct?.personalizableMarking == 1) {
      ...
      if (markingServices || markingServices.length > 0) {
        ...
        for (const data of dataMarkingServices) {
          ...
          if (!markingService) {
            throw new NotFoundException(`MarkingService with id ${data} not found`);
          }
          ...
          if (!quoteDetail) {
            throw new NotFoundException(`QuoteDetail with id ${quoteDetail} not found`);
          }

          // Update the MarkingService entity
          markingService.quoteDetail = quoteDetail;
        ...
        for (const markingService of markingServices) {
          ...
          for (const markedServicePrice of markingServiceProperty.markedServicePrices) {
            ...
            if (quantity >= markedServicePrice.minRange && quantity <= markedServicePrice.maxRange) {
              ...
              markingService.markingTransportPrice = markingTransportPrice || 0;
              markingService.calculatedMarkingPrice = totalMarking;
    ...
    updatedQuoteDetail.markingWithProductSupplierTransport = valorTransporteMarcacionx;
  ...
  async productSummary(calculateSummaryDto: CalculateSummaryDto, user: User) {
    ...
    const ClientCartQuote: User = await this.userRepository.findOne({
      where: {
    ...
    //* OBTENER LOS PRECIOS DE TRANSPORTE DEL PROVEEDOR AL CLIENTE : fedex
    ...
    totalPrice += ClientCartQuote?.client?.margin || 0;
    ...
    if (clientType.toLowerCase() == 'cliente corporativo secundario' || clientType.toLowerCase() == 'cliente corporativo principal') {
      const brandId = calculateSummaryDto?.brandId;
      console.log(brandId)
